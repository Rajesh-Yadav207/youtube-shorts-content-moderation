...
distance-based methods that suffer in the very high-dimensional (\~4,614-feature) space without more careful
tuning.
...
> matters, and \[`PROJECT\_REPORT.md` §8.2](PROJECT\_REPORT.md#82-label-taxonomies) for the exact label mapping.
...
numbers: some of the jump is a byproduct of one class getting bigger, and Section 6 of `PROJECT\_REPORT.md`
(§9.4) and the confusion-matrix discussion below both show Toxic's size inflating its raw correct-prediction
count, not just its separability.
...
For the full methodology and pipeline behind these numbers, see [`PROJECT\_REPORT.md`](PROJECT_REPORT.md). For
